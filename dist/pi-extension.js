...
import { createDrive9CodingAgentTools, createDrive9StorageOnlyBashOperations, createUnavailableCodingAgentTools, DRIVE9_STORAGE_ONLY_MESSAGE, } from "./pi-coding-agent.js";
import { DRIVE9_COMMAND_DESCRIPTION, registerDrive9Command, } from "./pi-extension-commands.js";
import { DRIVE9_EXTENSION_CONFIG_VERSION, getDrive9ProjectConfigPath, getDrive9ProjectTrustMarkerPath, resolveDrive9ExtensionConfig, writeDrive9ProjectConfig, } from "./pi-extension-config.js";
const DRIVE9_ROOT_FLAG = "drive9-root";
const NO_DRIVE9_FLAG = "no-drive9";
const DRIVE9_FILE_TOOL_NAMES = ["read", "write", "edit", "ls"];
...
function isDrive9FileTool(tool) {
    return DRIVE9_FILE_TOOL_NAMES.some((name) => name === tool.name);
}
function sameSourceInfo(left, right) {
...
function stateUsesDrive9Boundary(state) {
...
async function validateWorkspace(client, root) {
...
function statusText(state, context, activeTools) {
...
function resultStatusCode(error) {
...
function isMissingResult(error) {
    return (resultCode(error) === "not_found" ||
        resultStatusCode(error) === 404 ||
        errorMessage(error).toLowerCase().includes("not found"));
}
function isConflictResult(error) {
    const message = errorMessage(error).toLowerCase();
    return (resultStatusCode(error) === 409 ||
        message.includes("already exists") ||
        message.includes("exists:"));
}
async function createWorkspaceRoot(client, root) {
    ...
    for (const segment of normalized.split("/").filter(Boolean)) {
        ...
        try {
            ...
            if (!isMissingResult(error))
                throw error;
            try {
                await client.mkdir(current, 0o755);
            }
            catch (mkdirError) {
                if (!isConflictResult(mkdirError))
                    throw mkdirError;
            }
            info = await client.stat(current);
        }
...
async function verifyWrite(fileSystem) {
    const temporaryPath = getOrThrow(await fileSystem.createTempFile({ prefix: "verify-", suffix: ".txt" }));
    let operationError;
    let cleanupError;
    try {
        ...
        if (actual !== expected)
            throw new Error("Drive9 write verification read back different content");
    }
    catch (error) {
    ...
    if (operationError !== undefined) {
        if (cleanupError !== undefined) {
            throw new Error(`${errorMessage(operationError)}; temporary file cleanup failed at ${temporaryPath}: ${errorMessage(cleanupError)}`);
        }
...
export function createDrive9PiExtension(options = {}) {
    return (pi) => {
        const createClient = options.createClient ?? (() => Client.defaultClient());
        const environment = options.environment ?? process.env;
        let state = { mode: "inactive", reason: "unconfigured" };
        let toolOwnership;
        let previousBoundaryActiveTools;
        function captureBoundaryActiveTools() {
            previousBoundaryActiveTools ??= pi
                .getActiveTools()
                .filter((name) => DRIVE9_BOUNDARY_TOOL_NAMES.has(name));
        }
        function deactivateBoundaryTools() {
            pi.setActiveTools(pi.getActiveTools().filter((name) => !DRIVE9_BOUNDARY_TOOL_NAMES.has(name)));
        }
        function activateDrive9FileTools() {
            const nonBoundaryTools = pi
                .getActiveTools()
                .filter((name) => !DRIVE9_BOUNDARY_TOOL_NAMES.has(name));
            const activeFileTools = (previousBoundaryActiveTools ?? []).filter((name) => DRIVE9_FILE_TOOL_NAMES.some((fileToolName) => fileToolName === name));
            const defaultFileToolsWereActive = ["read", "write", "edit"].every((name) => activeFileTools.includes(name));
        ...
        function restoreBoundaryTools() {
            if (previousBoundaryActiveTools === undefined)
                return;
            const nonBoundaryTools = pi
                .getActiveTools()
                .filter((name) => !DRIVE9_BOUNDARY_TOOL_NAMES.has(name));
        ...
        function drive9CommandSourceInfo() {
            const commands = pi
                .getCommands()
                .filter((command) => command.source === "extension" &&
                command.description === DRIVE9_COMMAND_DESCRIPTION);
            if (commands.length !== 1) {
                throw new Error(`could not uniquely identify the Drive9 extension source (${commands.length} matching commands)`);
            }
            return commands[0].sourceInfo;
        }
        function ownershipMismatches(ownership, requiredToolName) {
            ...
            return DRIVE9_FILE_TOOL_NAMES.filter((name) => {
                ...
                if (effective === undefined)
                    return name === requiredToolName;
                return (effective.parameters !== ownership.parameters.get(name) ||
                    !sameSourceInfo(effective.sourceInfo, ownership.sourceInfo));
            });
        }
        function verifyToolOwnership(tools) {
            const ownership = {
            ...
            if (mismatches.length > 0) {
                throw new Error(`Drive9 does not own the effective ${mismatches.join(", ")} tool definition${mismatches.length === 1 ? "" : "s"}; disable the conflicting extension or SDK tool`);
            }
            return ownership;
        }
        function installUnavailableFileSystemTools(context, message) {
            for (const tool of createUnavailableCodingAgentTools(context.cwd, message)) {
                if (isDrive9FileTool(tool))
                    registerTool(pi, tool);
            }
        }
        function enterUnavailableState(resolved, context, error) {
        ...
        registerDrive9Command(pi, {
            ...
            setup: async (context, rootArgument) => {
                ...
                if (!context.isProjectTrusted()) {
                    context.ui.notify("Drive9 setup will not write an untrusted project. Use /trust, then restart Pi (or restart with --approve).", "error");
                    return;
                }
                const currentRoot = state.mode === "drive9" || state.mode === "checking" || state.mode === "error"
                    ? state.root
                    : undefined;
                const suggestedRoot = currentRoot ?? `/workspaces/${basename(context.cwd)}`;
                const enteredRoot = rootArgument ?? (await context.ui.input("Drive9 workspace root", suggestedRoot));
                if (enteredRoot === undefined)
                    return;
                ...
                try {
                    ...
                    try {
                        ...
                        if (resultCode(error) !== "not_found")
                            throw error;
                        const normalized = new Drive9FileSystem({ client, root: requestedRoot }).root;
                        const create = await context.ui.confirm("Create Drive9 workspace?", `${normalized} does not exist. Create it and any missing parent directories?`);
                        if (!create)
                            return;
                    ...
                    const configPath = getDrive9ProjectConfigPath(context.cwd);
                    const confirmed = await context.ui.confirm("Enable Drive9 for this project?", `Write ${configPath}\nRoot: ${fileSystem.root}`);
                    if (!confirmed)
                        return;
            ...
            disable: async (context) => {
                ...
                if (!context.isProjectTrusted()) {
                    context.ui.notify("Drive9 disable will not write an untrusted project. Use /trust, then restart Pi (or restart with --approve).", "error");
                    return;
                }
                ...
                if (state.source === "cli" || state.source === "env") {
                    context.ui.notify(`Drive9 is enabled by ${sourceLabel(state.source)}. Restart with --no-drive9 or remove that override.`, "warning");
                    return;
                }
                ...
                const confirmed = await context.ui.confirm("Disable Drive9?", `Restore Pi local tools for this project?\nRoot retained: ${state.root}`);
                if (!confirmed)
                    return;
                await context.waitForIdle();
                await writeDrive9ProjectConfig({
            ...
            verify: async (context, write) => {
                ...
                try {
                    ...
                    if (write)
                        await verifyWrite(fileSystem);
                    state = { ...state, checkedAt: new Date().toISOString() };
                    context.ui.notify(`Drive9 ${write ? "write" : "read"} verification succeeded: ${state.root}`, "info");
                }
                catch (error) {
        ...
        async function failInitialization(resolved, context, error) {
        ...
        pi.on("session_start", async (_event, context) => {
            if (state.mode === "drive9")
                await state.fileSystem.cleanup();
        ...
        pi.on("tool_call", async (event, context) => {
            if (state.mode === "checking" || state.mode === "error") {
                if (!DRIVE9_BOUNDARY_TOOL_NAMES.has(event.toolName))
                    return;
                return {
                    block: true,
            ...
            if (!DRIVE9_FILE_TOOL_NAMES.some((name) => name === event.toolName))
                return;
            const mismatches = toolOwnership === undefined
                ? [...DRIVE9_FILE_TOOL_NAMES]
                : ownershipMismatches(toolOwnership, event.toolName);
            if (mismatches.length === 0)
                return;
            const activeState = state;
            const ownershipError = new Error(`Drive9 lost ownership of the effective ${mismatches.join(", ")} tool definition${mismatches.length === 1 ? "" : "s"}; local fallback remains blocked`);
            const message = enterUnavailableState({
                status: "active",
                source: activeState.source,
                root: activeState.root,
            }, context, ownershipError);
            try {
        ...
        pi.on("user_bash", () => {
            if (!stateUsesDrive9Boundary(state))
                return;
        ...
        pi.on("before_agent_start", (event) => {
            if (state.mode === "error") {
                const boundary = "Drive9 is unavailable. Standard filesystem and process tools are blocked; do not access or modify the host filesystem.";
            ...
            const boundary = "Drive9 is storage-only in this session. Use read, write, edit, and ls. Bash, grep, find, and host filesystem access are unavailable.";
...
export default function drive9PiExtension(...args) {
