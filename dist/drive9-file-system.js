...
export class Drive9FileSystem {
    ...
    async operation(path, signal, operation) {
        const abort = this.aborted(signal, path === undefined ? undefined : this.safeAddress(path));
        if (abort !== undefined)
            return abort;
        try {
    ...
    async mutate(path, signal, operation) {
        const run = async () => await this.operation(path, signal, operation);
    ...
    async createTemporaryPath(prefix, suffix, directory, signal) {
...
//# sourceMappingURL=drive9-file-system.js.map