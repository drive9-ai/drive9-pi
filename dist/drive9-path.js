 ...
 */
export function normalizeDrive9PathText(value, label, allowEmpty, createError) {
    if (typeof value !== "string" || (!allowEmpty && value.length === 0)) {
        throw createError(`${label} must be ${allowEmpty ? "" : "a non-empty "}string`);
    }
    if (!isWellFormedUnicode(value))
        throw createError(`${label} must contain well-formed Unicode`);
...
export function normalizeDrive9AbsoluteRoot(value, label, createError) {
    ...
    if (normalized === "/")
        throw createError(`${label} cannot be the tenant root`);
    return normalized.endsWith("/") ? normalized.slice(0, -1) : normalized;
}
//# sourceMappingURL=drive9-path.js.map